...
class Bootstrap extends \Twig_Extension
{
	...
	public function getFunctions()
	{
		return [
			...
			new \Twig_SimpleFunction('modal', [$this, 'modal'], ['is_safe' => ['html']])
		];
	}
	...
	public function button($text, array $links, array $button = [], array $attr = [])
	{
		...
		return $this->getButton($text, $url, $button, $attr)
	...
	public function icon($symbol, $class = null, $library = 'glyphicon')
	{
		return TagBuilder::create('span')
			->addClass($library . ' ' . $library . '-' . $symbol)
			->addClass($class);
	}
	...
	{
		if ($url !== false) {
			$tag = TagBuilder::create('a')->attr('href', $url);
		}

		if (!$url) {
			$tag = TagBuilder::create('button')->attr('type', 'button');
		}
